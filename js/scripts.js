...
function setRandomBanner(){
	...
	var randomBannerId = Math.floor(Math.random() * 2);
	var randomBannerSrc = bannerArr[randomBannerId];

	$('#banner-place').attr('src',randomBannerSrc);

}

$('#banner-place').load(function(){
	checkBannerHeight();
});

function checkBannerHeight(){
	bannerHeight = $('#banner-block-1 .banner_box').height();
	if (bannerHeight>300){
		$('#bbf-1').css('display','none');
	} else {
		$('#bbf-1').css('display','block');
	}

	var windowWidth = $(window).width(); 
	
	bannerHeight = $('#bbx-2').height();
	if (bannerHeight>300){
		if (windowWidth>=768 && windowWidth<992){
			$('#ah-2').css({'display':'none'});
		} else {
			$('#ah-2').css({'display':'block'});
		}
	} else {
		$('#ah-2').css({'display':'block'});

		if (windowWidth>=768 && windowWidth<992){
			$('#ah-2').css('overflow','hidden');
		} else {
			$('#ah-2').css('overflow','visible');
		}
	}

	bannerHeight = $('#bbx-3').height();
	if (bannerHeight>300){
		if (windowWidth>=768 && windowWidth<992){
			$('#ah-3').css({'display':'none'});
		} else {
			$('#ah-3').css({'display':'block'});
		}
	} else {
		$('#ah-3').css({'display':'block'});

		if (windowWidth>=768 && windowWidth<992){
			$('#ah-3').css('overflow','hidden');
		} else {
			$('#ah-3').css('overflow','visible');
		}
	}
